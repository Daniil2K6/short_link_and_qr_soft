...
export const Statistics = () => {
  ...
  const handleGetStats = async () => {
    ...
    try {
      // Try to fetch by numeric ID first, then by short code
      let response = await fetch(`http://localhost:3000/api/links/${searchQuery}/stats`);
      
      // If ID doesn't work and it looks like a short code, try searching
      if (!response.ok && isNaN(searchQuery)) {
        const linksResponse = await fetch(`http://localhost:3000/api/links`);
        const linksData = await linksResponse.json();
        const link = linksData.links.find(l => l.shortCode === searchQuery);
        
        if (link) {
          response = await fetch(`http://localhost:3000/api/links/${link.id}/stats`);
        }
      }
      
      if (!response.ok) throw new Error('Link not found. Try using the link ID or short code from the dashboard.');
      const data = await response.json();
      setStats(data);
      
      // Generate QR code for the short link
      try {
        const qrResponse = await fetch(`http://localhost:3000/api/links/${data.id}/qrcode`);
        if (qrResponse.ok) {
